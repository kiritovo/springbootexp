...
import com.ice.example.util.base.JdbcDaoImpl;
import com.ice.example.util.base.Page;
import com.ice.example.util.base.Sql;
import com.mysql.jdbc.StringUtils;



@Repository   //持久层的注解
public class IceUserDaoImpl extends JdbcDaoImpl  implements  IceUserDao{
	...
	public Page<RoncooUser> queryForPage(int pageCurrent, int pageSize, String name) {
		// 确定参数
		/*
		 * String sql = "select * from roncoo_user where name=?"; return
		 * queryForPage(sql.toString(), pageCurrent, pageSize, RoncooUser.class,
		 * name);
		 */
		// 若name可能为空，则要进行判定，如下
		/*
		 * StringBuffer sql = new StringBuffer("select * from roncoo_user where
		 * 1"); if(!StringUtils.isNullOrEmpty(name)){ // Sql.checkSql
		 * 的作用是防止sql注入 sql.append(" and name =
		 * '").append(Sql.checkSql(name)).append("' "); } return
		 * queryForPage(sql.toString(), pageCurrent, pageSize,
		 * RoncooUser.class);
		 */
		// 若要like查询，如下
